...
"%PY_PATH%" -c "import re, os; p=r'%~dp0config.toml'; e=r'%~dp0config.toml.example'; text=open(p,encoding='utf-8').read() if os.path.exists(p) else ''; text=re.sub(r'api_key\s*=\s*\".*?\"', 'api_key = \"\"', text); text=re.sub(r'paired_tokens\s*=.*', 'paired_tokens = []', text); open(e,'w',encoding='utf-8').write(text)"
